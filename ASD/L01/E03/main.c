...
void InsertionSortDate(Tratta tratte[], int N);
void InsertionSortCodiceTratta(Tratta tratte[], int N);
void InsertionSortPartenza(Tratta tratte[], int N);
void InsertionSortCapolinea(Tratta tratte[], int N);
    ...
    do {
        ...
        switch(comando) {
            ...
            case r_ordinamento_data:
                InsertionSortDate(tratte, nTratte);
                for(int i = 0; i < nTratte; i++) {
                    stampaTratta(tratte[i]);
                }
                break;
            case r_ordinamento_codice_tratta:
                InsertionSortCodiceTratta(tratte, nTratte);
                for(int i = 0; i < nTratte; i++) {
                    stampaTratta(tratte[i]);
                }
                break;
            case r_ordinamento_stazione_partenza:
                InsertionSortPartenza(tratte, nTratte);
                for(int i = 0; i < nTratte; i++) {
                    stampaTratta(tratte[i]);
                }
                break;
            case r_ordinamento_stazione_arrivo:
                InsertionSortCapolinea(tratte, nTratte);
                for(int i = 0; i < nTratte; i++) {
                    stampaTratta(tratte[i]);
                }
                break;
            case r_ricerca_dicotomica:
                ...
                scanf("%s", filter);
                InsertionSortPartenza(tratte, nTratte);
                printf("%s\n", ricercaTrattaDicotomica(tratte, 0, nTratte-1, filter));
                break;
...
t_comandi leggiComando (void) {
    ...
    c = 0;
    char cmd[50];
    char tabella[50][30] = {"stampa", "ordinamento_data", "ordinamento_codice_tratta", "ordinamento_stazione_partenza", "ordinamento_stazione_arrivo", "ricerca_dicotomica", "ricerca_lineare"};
    do {
