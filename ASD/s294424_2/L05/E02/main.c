...
int main() {
    ...
    if(board_max == NULL) exit(-1);
    for(int i = 0; i < N_row; i++) {
        board_max[i] = malloc(sizeof(Tile) * N_col);
        if(board_max[i] == NULL) exit(-1);
...
void loadBoardAndTiles(Tile *** board, int * N_row, int * N_col, Tile ** collection_tiles, int * N_collection_tiles) {
    ...
    if(tmp_collection_tiles == NULL) exit(-1);
    int collection_tiles_id_used[*N_collection_tiles];
    ...
    if(collection_tiles_left == NULL) exit(-1); 
    int cnt = 0;
    for(int i = 0; i < *N_collection_tiles; i++) {
        int j = 0;
        int found = 0;
