...
CONTINUE:
    ...
    mul.d F5, F4, F3
    sub.d F5, F5, F6
    s.d F5, v4($t0) # DATA HAZARD: salvataggio in mem v4[i] = a*v1[i] - v2[i]; spostato qua
    ...
    daddi R4, R0, 3
    slt R5, R3, R4
    bnez R5, IS_DIVISIBLE_BY_THREE

    beq $t1, $t5, END

    daddi R4, R0, 4
    slt R5, R3, R4
    bnez R5, IS_NOT_DIVISIBLE_BY_THREE

    daddi R3, R3, 0
    J IS_DIVISIBLE_BY_THREE
