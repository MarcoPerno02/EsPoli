...
void configure_TIMER0(void) {
    ...
    uint32_t DutyCycle = 0X1E848;      // 50% Duty Cycle

		init_timer(0, Prescaler, 0, 0, 0xFF); // Configura MR0: reset su Periodo
...
void configure_TIMER1(void) {
    uint32_t Prescaler = 0;        // Prescaler per ottenere 1 MHz (100 ns per tick a 100 MHz)
    uint32_t Period = 0X9C4;        // Periodo totale: 0.1 ms (10000 Hz)
    uint32_t DutyCycle = 0X9C4;      // 100% Duty Cycle

		init_timer(1, Prescaler, 0, 3, 0x3E8); // Configura MR0: reset su Periodo
    //init_timer(1, Prescaler, 1, 1, DutyCycle); // Configura MR1: interrupt su ON
		enable_timer(1);
}
